...
for key in data.keys():
    item = data[key]
    single_insert = f"('{item.get('reference_number')}', '{item.get('website')}', '{item.get('court')}', '{item.get('judgment_date')}', '{item.get('publication_date')}', {item.get('mother_income')}, {item.get('father_income')}, {item.get('siblings')}),\n"
